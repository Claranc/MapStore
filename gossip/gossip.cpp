    ...
    while(1) {
        ...
        if(count < 10) {
            ...
            if(size > NUM+1) {
                ...
                for(int i = 0; i < NUM; i++) {
                    string send_data = "3";
                    string time0 = getTime();
                    send_data = send_data + time0 + "_";
...
//收到新加入节点时的处理函数
void Ring::process_hello(string x, sockaddr_in client_socket, int received,int sock) {
    //检测信息表中是否存在新节点数据
    int flag = 0;
...
//收到老大返回的消息时的处理函数
void Ring::process_respond(string x, sockaddr_in client_socket, int received,int sock) {
    //添加老大节点的信息
    ...
    s = NULL;
    //将老大返回的信息添加进来
    string x_data = x.substr(strlen("2"));
...
//收到正常心跳包时的处理函数
void Ring::process_info(string x, sockaddr_in client_socket, int received,int sock) {
    //获取系统当前时间
    string time_own = getTime();
    string time_origin = x.substr(1,x.find_first_of('_'));
    tm tm_1;
    time_t t_1; 
    const char *time1_1 = time_own.c_str();
    strptime(time1_1, "%Y/%m/%d %H:%M:%S", &tm_1);
    tm_1.tm_isdst = -1;  
    t_1  = mktime(&tm_1);
    //获取信息发送时的时间
    tm tm_2;
    time_t t_2; 
    const char *time2_1 = time_origin.c_str();
    strptime(time2_1, "%Y/%m/%d %H:%M:%S", &tm_2);
    tm_2.tm_isdst = -1; 
    t_2  = mktime(&tm_2);
    //时间相差过大表明网络延迟高，该包丢弃
    if(t_1 - t_2 < 3) {
        ...
        //对数据进行处理
        string x_data = x.substr(x.find_first_of('_')+1);
...
//将收到的信息进行分类，调用相应的函数进行处理
void Ring::ProcessData(string x, sockaddr_in client_socket, int received,int sock) {
    data_type msg_category;
    switch(x[0]) {
        //首字符为1,代表收到新加入节点消息
        case '1':
            msg_category = data_type::hello;
            break;
        //首字符为2,代表收到老大返回的消息
        case '2':
            msg_category = data_type::respond;
            break;
        //首字符为3,代表收到正常心跳包
        case '3':
            msg_category = data_type::info;
            break;
    }
    //选择调用相应的函数
    switch(msg_category) {
        case data_type::hello: {
            process_hello(x,client_socket, received, sock);
            break;  
        }
        case data_type::respond: {
            process_respond(x,client_socket, received, sock);
            break;
        }
        case data_type::info: {
             process_info(x,client_socket, received, sock);
        }
    }
}
